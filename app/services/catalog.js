...
.factory('catalog', ['$http', '$q', 'COOLSTORE_CONFIG', 'Auth', '$location', function($http, $q, COOLSTORE_CONFIG, $auth, $location) {
	...
	if ($location.protocol() === 'https') {
		baseUrl = (COOLSTORE_CONFIG.API_ENDPOINT.startsWith("https://") ? COOLSTORE_CONFIG.API_ENDPOINT : "https://" + COOLSTORE_CONFIG.API_ENDPOINT + '.' + $location.host().replace(/^.*?\.(.*)/g,"$1")) + '/api/products';
	} else {
